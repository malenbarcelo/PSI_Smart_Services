...
window.addEventListener('load',async()=>{
    ...
    //Define the table
    const tableInnerHTMLFirstLine = '<table class="table1"><tr><th class="tableTitle1 tableTitle2">Fecha</th><th class="tableTitle1">DNI</th><th class="tableTitle1 tableTitle1 tableTitle3">Apellido y nombre</th><th class="tableTitle1">Email</th><th class="tableTitle1">Nota</th></tr>'
    ...
    orderDate.addEventListener("click",async(e)=>{

        const prueba = await getData(course,company,'allDAta','noOrder')

        console.log(prueba)

        if (viewAllData.classList.contains('underlined')) {
            const data = await getData(course,company,'allDAta','orderDate')
        }
        if (viewPassed.classList.contains('underlined')) {
            console.log('hola')
        }
        if (viewNotPassed.classList.contains('underlined')) {
            console.log('hola')
        }
    })
})